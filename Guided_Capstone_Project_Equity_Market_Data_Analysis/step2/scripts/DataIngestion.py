...
class DataIngestion:
    def __init__(self, spark, container, storage_account, key):
        self.spark = spark
    ...
    def write_partitioned_parquet(self, df, partition_key, mode, output_dir):
        df.write.partitionBy(partition_key).mode(mode).parquet(
            self._make_path("", f"output/common_events/{output_dir}/")
        )
