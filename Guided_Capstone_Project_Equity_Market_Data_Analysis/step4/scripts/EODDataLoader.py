...
class EODDataLoader(StorageHandler):
    ...
    def _load_quotes(self):
        ...
        quote_common = self.spark.read.parquet(self._make_path(f"{COMMON_EVENTS_OUTPUT_CSV}/partition=Q"),
        self._make_path(f"{COMMON_EVENTS_OUTPUT_JSON}/partition=Q"))
       
        quote = quote_common.select(F.col("trade_dt"), F.col("symbol"), F.col("exchange"), F.col("event_tm"),
                                    F.col("event_seq_nb"), F.col("arrival_tm"), F.col("bid_pr"))
        
        quote_corrected = self._apply_latest(quote, ["trade_dt", "symbol", "exchange", "event_tm", "event_seq_nb"], "arrival_tm")
