...
from Constants import RAW_CSV_PATH, RAW_JSON_PATH, COMMON_EVENTS_OUTPUT_CSV, COMMON_EVENTS_OUTPUT_JSON, FINAL_TRADE_OUTPUT, FINAL_QUOTE_OUTPUT, FINAL_QUOTE_TRADE_ANALYTICAL_OUTPUT
...
class ETLPipeline:
    def __init__(self, spark, storage_cfg):
        ...
        self.analyzer = DataAnalyzer(spark, storage_cfg)

    def run(self):
        # Step 1: ingest raw data
        #df_csv = self.ingestor.read_csv(RAW_CSV_PATH)
        #df_json = self.ingestor.read_json(RAW_JSON_PATH)
        ...
        self.analyzer.analyze_trade_data("trade", "2020-08-06")
        final_output = self.analyzer.enrich_quote_data("quote", "2020-08-06")
        self.analyzer.write_partitioned_parquet(final_output, "trade_dt", "overwrite", FINAL_QUOTE_TRADE_ANALYTICAL_OUTPUT)