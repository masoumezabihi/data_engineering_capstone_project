...
class Transformer:
    ...
    def drop_columns(self, df, missing_values_col, missing_threshold = 95):
        """
        Drops specified columns from DataFrame if present.
        Handles both a list of column names or a DataFrame with column_name/percent_missing.
        """
        # Case 1: If input is a list of column names
        if isinstance(missing_values_col, list):
            cols_to_drop = [c for c in missing_values_col if c in df.columns]

        # Case 2: If input is a DataFrame with percent_missing
        else:
            Miss_Vars_df = (
                missing_values_col
                .filter(missing_values_col.percent_missing > missing_threshold)
                .select("column_name")
            )
            cols_to_drop = [row["column_name"] for row in Miss_Vars_df.collect()
                            if row["column_name"] in df.columns]

        Logger.log(f"Dropping columns: {cols_to_drop}")
    ...
    def find_column_missing_percentage(self, df: DataFrame):
        """
